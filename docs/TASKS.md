...
- - [ ] **Task ID:** P4-T1
  ...
  - **Subtasks:**
    ...
    - [ ] **Subtask ID:** P4-T1.4
      - **Name:** Integrate Meshing into Asynchronous Chunk Generation Task
      - **Description:** Modify the existing `ChunkGenerationTask` (from P3-T6.4). After the block data for a chunk has been generated by the `TerrainGenerator`, invoke the newly implemented meshing algorithm (from P4-T1.2 and P4-T1.3). The generated `ChunkMeshData` (from P4-T1.1) should be stored, likely within the `Chunk` object itself or closely associated with it, so it can be accessed by the rendering thread. Update the `ChunkState` of the chunk to `MESHED` upon successful mesh generation.
      - **Dependencies:** P4-T1.3, P3-T6 (ChunkGenerationTask), P3-T1 (ChunkState, Chunk object modification)
    - [ ] **Subtask ID:** P4-T1.6
